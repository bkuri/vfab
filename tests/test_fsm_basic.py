...
class TestBasicFSM:
    ...
    def fsm(self, mock_workspace):
        ...
        with patch('plotty.fsm.create_hook_executor', return_value=Mock()), \
    ...
    def test_workflow_progression(self, fsm):
        ...
        assert fsm.transition_to(target_state=JobState.ANALYZED, reason="Analysis complete") is True
        assert fsm.current_state == JobState.ANALYZED
        
        assert fsm.transition_to(target_state=JobState.OPTIMIZED, reason="Optimization complete") is True
        assert fsm.current_state == JobState.OPTIMIZED
        
        assert fsm.transition_to(target_state=JobState.READY, reason="Ready to plot") is True
