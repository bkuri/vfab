    ...
    def test_prompt_pen_swap_invalid_then_valid(self):
        ...
        with patch('builtins.input', side_effect=['invalid', 'continue']):
            with patch('builtins.print') as mock_print:
                ...
                mock_print.assert_any_call("Invalid option. Try: continue, skip, or abort")
    ...
    def test_multipen_plotter_initialization(self, mock_manager):
        ...
        with patch('plotty.plotting.is_axidraw_available', return_value=True):
            with patch('plotty.plotting.create_manager', return_value=mock_manager):
                plotter = MultiPenPlotter(port="/dev/ttyUSB0", model=2, interactive=False)
    ...
    def test_plot_multipen_job_success(self, mock_manager, workspace):
        ...
        with patch('plotty.plotting.is_axidraw_available', return_value=True):
            with patch('plotty.plotting.create_manager', return_value=mock_manager):
                ...
                layers = [
                    {"name": "layer_0", "svg": str(layer_files[0]), "order_index": 0, "element_count": 10},
                    {"name": "layer_1", "svg": str(layer_files[1]), "order_index": 1, "element_count": 15},
                    {"name": "layer_2", "svg": str(layer_files[2]), "order_index": 2, "element_count": 20}
                ]
                ...
                assert result["pen_swaps"] == 2  # Should swap pen for each layer after first
    ...
    def test_plot_multipen_job_with_pen_swaps(self, mock_manager, workspace):
        ...
        with patch('plotty.plotting.is_axidraw_available', return_value=True):
            with patch('plotty.plotting.create_manager', return_value=mock_manager):
                ...
                layers = [
                    {"name": "layer_0", "svg": str(layer_file), "order_index": 0, "element_count": 10},
                    {"name": "layer_1", "svg": str(layer_file), "order_index": 1, "element_count": 15}
                ]
    ...
    def test_plot_multipen_job_missing_layer_file(self, mock_manager, workspace):
        ...
        with patch('plotty.plotting.is_axidraw_available', return_value=True):
            with patch('plotty.plotting.create_manager', return_value=mock_manager):
                ...
                layers = [
                    {"name": "missing_layer", "svg": str(job_dir / "missing.svg"), "order_index": 0}
                ]
    ...
    def test_plot_with_axidraw_layers_success(self, mock_manager, workspace):
        ...
        with patch('plotty.plotting.is_axidraw_available', return_value=True):
            with patch('plotty.plotting.create_manager', return_value=mock_manager):
                ...
                with patch('plotty.plotting.detect_svg_layers', return_value=mock_layers):
                    with patch('plotty.plotting.parse_axidraw_layer_control') as mock_parse:
                        mock_control = Mock()
