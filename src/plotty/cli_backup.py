...
def plan(
    ...
    try:
        ...
        validate_file_exists(svg_path, "Source SVG file")

        with progress.spinner("Loading available pens..."):
            ...
            try:
                ...
                logger.debug(f"Failed to load pen configuration: {e}")

        # Import multipen functions for layer detection
        from .multipen import detect_svg_layers, display_layer_overview

        # Detect layers with progress
        with progress.spinner("Detecting SVG layers..."):
...
def record_test(
    ...
    try:
        ...
        with progress.spinner(f"Recording {seconds} seconds of video..."):
...
def plot(
    ...
    try:
        ...
        if not plan_file.exists():
            raise typer.BadParameter(
                f"Job {job_id} not planned. Run 'plan' command first."
            )
        ...
        if multipen and plan_data.get("layer_count", 0) > 1:
            ...
            with progress.spinner("Initializing multi-pen plotter..."):
                plotter = MultiPenPlotter(
                    port=port, model=model, interactive=interactive
                )

            if (
                plan_data.get("multipen_svg")
                and Path(plan_data["multipen_svg"]).exists()
            ):
                # Use AxiDraw native layer control
                with layer_progress.process_layers(
                    plan_data["layer_count"], "Plotting"
                ):
                    result = plotter.plot_with_axidraw_layers(
                        Path(plan_data["multipen_svg"])
                    )
            else:
            ...
            validate_file_exists(svg_file, "Source SVG file")

            with progress.spinner("Initializing plotter..."):
                manager = create_manager(port=port, model=model)

            mode = "preview" if preview else "plotting"
            with progress.spinner(f"{mode.capitalize()} job..."):
...
def interactive(port: Optional[str] = None, model: int = 1, units: str = "inches"):
    ...
    try:
        ...
        with progress.spinner("Connecting to AxiDraw..."):
            manager = create_manager(port=port, model=model)
...
def pen_test(port: Optional[str] = None, model: int = 1, cycles: int = 3):
    ...
    try:
        ...
        with progress.spinner("Initializing plotter for pen test..."):
            manager = create_manager(port=port, model=model)

        print(f"Testing pen movement ({cycles} cycles)...")

        with progress.task("Testing pen movement", total=cycles) as update:
            for i in range(cycles):
                update(1, f"Cycle {i + 1}/{cycles}")
...
def pen_list():
    ...
    try:
        ...
        with progress.spinner("Loading pens from database..."):
            console = Console()
            ...
            with get_session() as session:
                ...
                for pen in pens:
                    ...
                    if pen.name == default_pen:
                        console.print(
                            f"  {pen.id}: [green]{pen.name}[/green] (default)"
                        )
                    else:
...
def pen_add(
    ...
    try:
        ...
        with progress.spinner("Adding pen to database..."):
            with get_session() as session:
...
def pen_remove(name: str):
    ...
    try:
        ...
        with progress.spinner("Removing pen from database..."):
            with get_session() as session:
...
def report(
    ...
    try:
        ...
        with progress.spinner("Generating job report..."):
...
def paper_list():
    ...
    try:
        ...
        with progress.spinner("Loading paper sizes..."):
            console = Console()
            cfg = load_config(None)
            ...
            try:
                ...
                logger.debug(f"Failed to load paper configuration: {e}")
...
def paper_add(
    ...
    try:
        ...
        with progress.spinner("Adding custom paper size..."):
            with get_session() as session:
...
def paper_remove(name: str):
    ...
    try:
        ...
        with progress.spinner("Removing custom paper..."):
            with get_session() as session:
...
def recovery_list():
    ...
    try:
        ...
        with progress.spinner("Scanning for recoverable jobs..."):
...
def recovery_recover(job_id: str):
    ...
    try:
        ...
        with progress.spinner(f"Recovering job {job_id}..."):
...
def recovery_recover_all():
    ...
    try:
        ...
        with progress.spinner("Recovering all jobs..."):
...
def recovery_status(job_id: str):
    ...
    try:
        ...
        with progress.spinner(f"Getting recovery status for {job_id}..."):
            status = recovery.get_job_status(job_id)
...
def recovery_cleanup(job_id: str, keep_entries: int = 100):
    ...
    try:
        ...
        with progress.spinner(f"Cleaning journal for job {job_id}..."):
