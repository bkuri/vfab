"""
Initialization utilities for ploTTY configuration and workspace setup.
...
def get_xdg_config_dir() -> Path:
    """Get XDG config directory for ploTTY."""
    return Path(platformdirs.user_config_dir("plotty"))


def get_xdg_data_dir() -> Path:
    """Get XDG data directory for ploTTY."""
    return Path(platformdirs.user_data_dir("plotty"))
...
def install_default_config(force: bool = False) -> bool:
    ...
    try:
        ...
        try:
            ...
            package_dir = Path(plotty.__file__).parent
            source_locations.append(package_dir / "config")
        except (ImportError, AttributeError):
            pass

        # 2. Local development path
        source_locations.append(Path("src/plotty/config"))
...
def is_first_run() -> bool:
    """
    Check if this is the first time ploTTY is run.
...
def initialize_plotty(force_config: bool = False) -> bool:
    """
    Initialize ploTTY configuration and workspace.
...
def get_user_config_path() -> Path:
    """Get the user config file path, respecting PLOTTY_CONFIG env var."""
    if "PLOTTY_CONFIG" in os.environ:
        return Path(os.environ["PLOTTY_CONFIG"])
