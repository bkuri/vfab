#!/usr/bin/env python3
"""
ploTTY TUI with Full Backend Integration

Enhanced TUI that connects to real ploTTY backend systems:
...
class LayerProgressBar(Vertical):
    ...
    def __init__(self, total_layers: int = 1, **kwargs):
        ...
        self.layer_progress = [0.0] * total_layers
...
class IntegratedSessionWidget(Vertical):
    """Enhanced session widget with backend integration."""
...
class IntegratedDashboardTab(TabPane):
    ...
    async def on_button_pressed(self, event: Button.Pressed) -> None:
        ...
        if event.button.id == "connect-backend":
...
class IntegratedPloTTYTUIApp(App):
    """ploTTY TUI with full backend integration."""
    ...
    def compose(self) -> ComposeResult:
        ...
        with TabbedContent(id="main-tabs"):
            yield IntegratedDashboardTab(
                self.backend_manager, title="📊 Dashboard", id="dashboard-tab"
            )
    ...
    async def action_add_job(self) -> None:
        ...
        try:
            ...
            if idle_devices:
                ...
                self._log_message(f"Added job {job_id} to {device.name}")
            else:
...
async def main():
    """Run the integrated ploTTY TUI."""
    print("Starting ploTTY TUI with Backend Integration")
    print("=" * 60)
    print("Features:")
