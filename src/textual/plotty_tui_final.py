#!/usr/bin/env python3
"""
ploTTY TUI - Final Integrated Version

Complete TUI with real backend integration and all features:
- Real backend connectivity (mock, database, WebSocket)
- Device health monitoring
- Layer-by-layer progress visualization
- System logging
- Full FSM state management
- Real-time updates
"""
...
from plotty_backend_connector import (
    PloTTYBackendManager,
    Device,
    Job,
    DeviceType,
    ConnectionState,
...
class LayerProgressBar(Vertical):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical(id="layers-container"):
            for i in range(min(self.total_layers, 10)):  # Limit to 10 visible layers
                yield ProgressBar(
    ...
    def update_progress(self, current_layer: int, overall_progress: float) -> None:
        ...
        # Update layer progress (show up to 10 layers around current)
        visible_layers = min(self.total_layers, 10)
        start_layer = max(0, current_layer - 5)

        for i in range(visible_layers):
            actual_layer = start_layer + i
            if actual_layer >= self.total_layers:
                break

            try:
                layer_bar = self.query_one(f"#layer-{i}", ProgressBar)
                if actual_layer < current_layer:
                    layer_bar.progress = 100.0
                elif actual_layer == current_layer:
...
class IntegratedSessionWidget(Vertical):
    """Enhanced session widget with real backend integration."""

    def __init__(
        self,
        device: Device,
        backend_manager: PloTTYBackendManager,
        **kwargs,
    ):
        super().__init__(**kwargs)
        self.device = device
    ...
    async def on_button_pressed(self, event: Button.Pressed) -> None:
        ...
        if not backend or not self.current_job:
            # Fallback to mock behavior if no backend
            await self._handle_mock_button_press(button_id)
            return

        try:
            if button_id == "start" and self.state in ["READY", "ARMED", "PAUSED"]:
                await backend.update_job_state(self.current_job.id, "PLOTTING")

            elif button_id == "pause" and self.state == "PLOTTING":
                await backend.update_job_state(self.current_job.id, "PAUSED")
                ...
                await backend.update_job_state(self.current_job.id, "ABORTED")
                ...
                await backend.update_job_state(self.current_job.id, "QUEUED")

            elif button_id == "reset" and self.state in COMPLETED_STATES:
                await backend.update_job_state(self.current_job.id, "NEW")
        except Exception as e:
            # Fallback to mock behavior on error
            await self._handle_mock_button_press(button_id)

    async def _handle_mock_button_press(self, button_id: str) -> None:
        """Handle button press with mock behavior."""
        if button_id == "start" and self.state in ["READY", "ARMED", "PAUSED"]:
            self.state = "PLOTTING"
    ...
    async def _simulate_progress(self) -> None:
        ...
        if not self.current_job:
            ...
            self.total_layers = 5
            self.job_name = (
                self.current_job.name if self.current_job else "test-job.svg"
            )
...
class IntegratedDashboardTab(TabPane):
    """Enhanced dashboard with real backend integration."""

    def __init__(self, backend_manager: PloTTYBackendManager, **kwargs):
        kwargs.pop("title", None)  # Remove title if present
        super().__init__(title="📊 Dashboard", **kwargs)
        self.backend_manager = backend_manager
        self.session_widgets: Dict[str, IntegratedSessionWidget] = {}

    def compose(self) -> ComposeResult:
        """Create dashboard content."""
        yield Label("ploTTY Session Manager", classes="dashboard-title")

        with Horizontal(classes="connection-status"):
            yield Label("Backend:", classes="status-label")
            yield Label("Disconnected", id="backend-status", classes="status-value")
            yield Button("Connect", id="connect-backend", variant="primary")
    ...
    def _update_connection_status(self) -> None:
        ...
        try:
            status_label = self.query_one("#backend-status", Label)
            connect_button = self.query_one("#connect-backend", Button)

            state = self.backend_manager.get_connection_state()
    ...
    async def _update_system_health(self) -> None:
        ...
        try:
            # Get health for first device as system health
            devices = await backend.get_devices()
            if devices:
                health = await backend.get_device_health(devices[0].id)
            ...
            self._log_message(f"Health update failed: {e}")
    ...
    def update_stats(self, devices: List[Device], jobs: List[Job]) -> None:
        """Update dashboard statistics."""
        active_count = sum(1 for d in devices if d.is_active)
        idle_count = len(devices) - active_count
        total_count = len(devices)
        jobs_count = len([j for j in jobs if j.state not in COMPLETED_STATES])
        ...
        try:
            ...
            jobs_label.update(f"Jobs: {jobs_count}")
        except:
            pass
    ...
    async def create_session_widget(self, device: Device) -> None:
        ...
        session_widget = IntegratedSessionWidget(
            device=device,
    ...
    async def update_session_from_job(self, job: Job) -> None:
        """Update session widget from job data."""
        session_widget = self.session_widgets.get(job.device_id)
        if session_widget:
            session_widget.update_from_job(job)

    async def on_button_pressed(self, event: Button.Pressed) -> None:
        """Handle dashboard button presses."""
        button_id = event.button.id
        if button_id == "connect-backend":
            await self._connect_backend()

    async def _connect_backend(self) -> None:
        """Connect to backend."""
        self._log_message("Connecting to ploTTY backend...")

        try:
            success = await self.backend_manager.initialize("mock")
            if success:
                self._log_message("Successfully connected to backend")
                self._update_connection_status()

                # Load initial data
                await self._load_initial_data()
            else:
                self._log_message("Failed to connect to backend")
        except Exception as e:
            self._log_message(f"Connection error: {e}")

    async def _load_initial_data(self) -> None:
        """Load initial devices and jobs."""
        backend = self.backend_manager.get_backend()
        if not backend:
            return
        ...
        try:
            ...
            # Load jobs and update sessions
            jobs = await backend.get_jobs()
            for job in jobs:
                await self.update_session_from_job(job)

            # Update stats
            self.update_stats(devices, jobs)

            self._log_message(f"Loaded {len(devices)} devices and {len(jobs)} jobs")

        except Exception as e:
            self._log_message(f"Failed to load initial data: {e}")


class FinalPloTTYTUIApp(App):
    """ploTTY TUI with complete backend integration."""
    ...
    IntegratedSessionWidget.state-plotting {
        background: $success;
        color: $text;
        border: solid $success;
    }

    IntegratedSessionWidget.state-paused {
        background: $warning;
        color: $text;
        border: solid $warning;
    }

    IntegratedSessionWidget.state-completed {
        background: $success-muted;
    }

    IntegratedSessionWidget.state-failed, IntegratedSessionWidget.state-aborted {
        background: $error;
        color: $text;
        border: solid $error;
    }
    ...
    BINDINGS = [
        ...
        Binding("q", "quit", "Quit"),
        Binding("c", "connect_backend", "Connect Backend"),
    ...
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.backend_manager = PloTTYBackendManager()
    ...
    def compose(self) -> ComposeResult:
        ...
        with TabbedContent(id="main-tabs"):
            yield IntegratedDashboardTab(self.backend_manager, id="dashboard-tab")

        yield Footer()

    async def on_mount(self) -> None:
        """Initialize the application."""
        # Set up backend update callback
        backend = self.backend_manager.get_backend()
        if backend:
            backend.set_update_callback(self._handle_backend_update)

    async def _handle_backend_update(
        self, update_type: str, data: Dict[str, Any]
    ) -> None:
        """Handle real-time updates from backend."""
        try:
            dashboard = self.query_one("#dashboard-tab", IntegratedDashboardTab)

            if update_type == "job_added":
                job_data = data.get("job", {})
                job = Job(**job_data)
                await dashboard.update_session_from_job(job)
                self._log_message(f"Job added: {job.name}")

            elif update_type == "job_updated":
                job_data = data.get("job", {})
                job = Job(**job_data)
                await dashboard.update_session_from_job(job)
                ...
                if old_state != new_state:
                    self._log_message(f"Job {job.name}: {old_state} → {new_state}")

            elif update_type == "device_updated":
                device_id = data.get("device_id")
                self._log_message(f"Device updated: {device_id}")

            elif update_type == "health_updated":
                device_id = data.get("device_id")
                self._log_message(f"Health updated: {device_id}")

        except Exception as e:
            self._log_message(f"Update handling error: {e}")

    def _log_message(self, message: str) -> None:
        """Log a message to the system log."""
        try:
            dashboard = self.query_one("#dashboard-tab", IntegratedDashboardTab)
            dashboard._log_message(message)
        except:
            pass

    async def action_connect_backend(self) -> None:
        """Connect to backend."""
        try:
            dashboard = self.query_one("#dashboard-tab", IntegratedDashboardTab)
            await dashboard._connect_backend()
        except:
            pass
    ...
    async def action_refresh(self) -> None:
        ...
        try:
            dashboard = self.query_one("#dashboard-tab", IntegratedDashboardTab)
            await dashboard._load_initial_data()
            self._log_message("Data refreshed")
        except:
            pass
    ...
    async def action_add_job(self) -> None:
        ...
        backend = self.backend_manager.get_backend()
        if not backend:
            self._log_message("Backend not connected")
            return
        ...
        try:
            ...
            if idle_devices:
                ...
                job_id = await backend.add_job(
                    {
                        "name": f"manual-job-{int(time.time())}.svg",
                        "device_id": device.id,
                        "total_layers": 3,
                    }
                )
                device_name = device.name if device else "Unknown Device"
                self._log_message(f"Added job {job_id} to {device_name}")
            else:
                self._log_message("No idle devices available")
        except Exception as e:
            self._log_message(f"Failed to add job: {e}")
...
async def main():
    """Run the final ploTTY TUI."""
    print("Starting ploTTY TUI - Final Integrated Version")
    print("=" * 60)
    print("Features:")
    print("- 🔄 Real backend integration (mock/database/WebSocket)")
