...
def get_current_version() -> str:
    ...
    result = run_command(
        "uvx python -c \"import tomllib; data=tomllib.load(open('pyproject.toml', 'rb')); print(data['project']['version'])\""
    )
...
def generate_changelog() -> str:
    ...
    for commit in commits:
        ...
        if any(keyword in message.lower() for keyword in ["feat", "feature", "add"]):
            ...
            keyword in message.lower() for keyword in ["improve", "optimize", "enhance"]
        ):
            categories["🔧 Improvements"].append(f"- {message} ({hash_short})")
        elif any(
            keyword in message.lower() for keyword in ["doc", "readme", "changelog"]
        ):
