...
This guide covers recovery procedures, crash analysis, and preventive measures for handling system failures and interruptions in ploTTY.
...
plotty info system | grep -i signal
...
```bash
# Check for crashes
grep -i "crash\|error\|exception" ~/.local/share/plotty/logs/plotty.log

# Check system logs
journalctl -u plotty --since "1 hour ago"
...
# Check memory usage
free -h
ps aux | grep plotty | awk '{sum+=$6} END {print sum/1024 "MB"}'

# Check file sizes
find ~/.local/share/plotty/workspace -name "*.svg" -exec ls -lh {} \; | sort -k5 -hr
```
...
for job in $(plotty info queue --state plotting | awk '{print $1}'); do
    plotty pause $job
done

# Save current state
plotty system export --output emergency_backup_$(date +%Y%m%d_%H%M%S).tar.gz
```
...
# Test device
plotty check servo --force-detect
...
# Repair if needed
sqlite3 ~/.local/share/plotty/plotty.db ".recover" | sqlite3 recovered.db
mv recovered.db ~/.local/share/plotty/plotty.db
```

**Database Corruption Recovery:**
```bash
# Backup corrupted database
cp ~/.local/share/plotty/plotty.db ~/.local/share/plotty/plotty.db.corrupted

# Reinitialize from backup
plotty system import --backup latest_backup.tar.gz
...
for journal in ~/.local/share/plotty/workspace/jobs/*/journal.jsonl; do
...
grep "emergency_shutdown" ~/.local/share/plotty/workspace/jobs/*/journal.jsonl | \
...
cp ~/.local/share/plotty/workspace/jobs/<job_id>/journal.jsonl \
   ~/.local/share/plotty/workspace/jobs/<job_id>/journal.jsonl.backup

# Extract valid entries
grep -v "invalid_json" ~/.local/share/plotty/workspace/jobs/<job_id>/journal.jsonl.backup > \
   ~/.local/share/plotty/workspace/jobs/<job_id>/journal.jsonl
...
mkdir -p ~/.local/share/plotty/workspace/jobs/<job_id>
cat > ~/.local/share/plotty/workspace/jobs/<job_id>/journal.jsonl << EOF
...
Valid transitions in ploTTY FSM:
```
...
```bash
# Check current state
plotty info job <job_id> --state-only

# Reset to NEW if in invalid state
plotty remove job <job_id> --force
plotty add job <job_id> <original_file.svg>
...
```bash
# Identify stuck jobs
plotty info queue --stuck

# Force state transition (advanced)
sqlite3 ~/.local/share/plotty/plotty.db "UPDATE jobs SET state = 'READY' WHERE id = '<job_id>'"
...
interrupted_jobs=$(plotty info queue --interrupted | awk '{print $1}')
...
for job_id in $interrupted_jobs; do
    ...
    # Check job state
    current_state=$(plotty info job $job_id --state-only)
    ...
    # Verify recovery
    new_state=$(plotty info job $job_id --state-only)
...
cp ~/.config/plotty/config.yaml ~/config_backup_$(date +%Y%m%d_%H%M%S).yaml

# Database backup
cp ~/.local/share/plotty/plotty.db ~/database_backup_$(date +%Y%m%d_%H%M%S).db
```

**Restore from Backup:**
```bash
# Restore full system
plotty system import --backup emergency_backup_YYYYMMDD_HHMMSS.tar.gz

# Restore configuration
cp ~/config_backup_YYYYMMDD_HHMMSS.yaml ~/.config/plotty/config.yaml

# Restore database
cp ~/database_backup_YYYYMMDD_HHMMSS.db ~/.local/share/plotty/plotty.db
```
...
mv jobs/<job_id> ~/.local/share/plotty/workspace/jobs/

# Update database
sqlite3 ~/.local/share/plotty/plotty.db "INSERT OR REPLACE INTO jobs VALUES ('<job_id>', 'RECOVERED', datetime('now'), datetime('now'));"
```
...
with open('~/.config/plotty/config.yaml') as f:
    current = yaml.safe_load(f)

# Merge backup into current (backup takes precedence)
current.update(backup)

with open('~/.config/plotty/config.yaml', 'w') as f:
    yaml.dump(current, f)
"
```
...
```bash
# Symptoms
grep "out of memory\|MemoryError" ~/.local/share/plotty/logs/plotty.log

# Analysis
free -h
ps aux | grep plotty | sort -k4 -nr

# Solution
# Reduce job complexity or increase system memory
plotty add job large_job file.svg --optimization fast
```

**Pattern 2: Device Disconnection**
```bash
# Symptoms
grep "device.*disconnect\|connection.*lost" ~/.local/share/plotty/logs/plotty.log
...
plotty check servo --reconnect
```

**Pattern 3: Database Locks**
```bash
# Symptoms
grep "database.*lock\|locked" ~/.local/share/plotty/logs/plotty.log
...
grep -E "(emergency_shutdown|ERROR|CRITICAL)" ~/.local/share/plotty/logs/plotty.log | \
...
grep -E "(emergency_shutdown|ERROR)" ~/.local/share/plotty/logs/plotty.log | \
...
while true; do
    echo "$(date): $(ps aux | grep plotty | grep -v grep | awk '{sum+=$3} END {print sum}')% CPU, $(ps aux | grep plotty | grep -v grep | awk '{sum+=$6} END {print sum/1024}')MB RAM"
    sleep 5
done &

# Run job and monitor
plotty restart <problematic_job>
```
...
LOG_FILE="/var/log/plotty_monitor.log"

while true; do
    timestamp=$(date '+%Y-%m-%d %H:%M:%S')
    
    # Check for stuck jobs
    stuck_jobs=$(plotty info queue --stuck | wc -l)
    if [ $stuck_jobs -gt 0 ]; then
        echo "[$timestamp] WARNING: $stuck_jobs stuck jobs detected" >> $LOG_FILE
    fi
    
    # Check disk space
    disk_usage=$(df ~/.local/share/plotty/ | tail -1 | awk '{print $5}' | sed 's/%//')
    if [ $disk_usage -gt 90 ]; then
        echo "[$timestamp] CRITICAL: Disk usage ${disk_usage}%" >> $LOG_FILE
    fi
    
    # Check for errors
    error_count=$(grep -c "ERROR" ~/.local/share/plotty/logs/plotty.log)
...
interrupted=$(plotty info queue --interrupted | awk '{print $1}')
...
if [ ! -z "$interrupted" ]; then
    ...
    echo "$(date): Auto-recovery completed" >> /var/log/plotty_auto_recovery.log
fi
```
...
# System backup
plotty system export --output $BACKUP_DIR/system_$DATE.tar.gz
...
# Clean old logs
find ~/.local/share/plotty/logs -name "*.log.*" -mtime +7 -delete

# Clean old journals
find ~/.local/share/plotty/workspace/jobs -name "journal.jsonl" -mtime +30 -exec sh -c '
    file="$1"
    backup="${file}.backup"
    # Keep last 50 lines
    tail -50 "$file" > "$backup"
...
find ~/.local/share/plotty/workspace/jobs -name "job.json" -mtime +30 -exec dirname {} \; | \
    xargs -I {} rm -rf {}
...
if ! python -c "import yaml; yaml.safe_load(open('~/.config/plotty/config.yaml'))" 2>/dev/null; then
    echo "ERROR: Invalid configuration file"
    exit 1
fi

# Check workspace
if [ ! -d ~/.local/share/plotty/workspace ]; then
    echo "ERROR: Workspace directory not found"
    exit 1
fi

# Check database
if [ ! -f ~/.local/share/plotty/plotty.db ]; then
    echo "ERROR: Database file not found"
    exit 1
fi

# Check device access
if ! plotty check servo >/dev/null 2>&1; then
...
```bash
# Immediate recovery
plotty resume                           # Resume all interrupted jobs
plotty restart <job_id>                 # Restart specific job
plotty check self                       # System health check

# Data recovery
plotty system export --output backup.tar.gz
plotty system import --backup backup.tar.gz

# Cleanup
plotty cleanup --jobs --older-than 30d
plotty cleanup --logs --older-than 7d
```
...
Job Journals:    ~/.local/share/plotty/workspace/jobs/*/journal.jsonl
```
...
This comprehensive recovery guide ensures that ploTTY users can handle any crash scenario effectively and minimize downtime.