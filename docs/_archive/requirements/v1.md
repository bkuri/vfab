# vfab v1 — Product Requirements Document (COMPLETED)
...
8. ✅ **Statistics** → comprehensive analytics with `vfab stats summary`, `vfab stats jobs`, `vfab stats performance`.
...
- CLI (vfab) + daemon (vfabd) with local JSON-RPC; logs to stdout + JSONL.
...
**CLI Commands:**
```bash
vfab stats summary [--json]     # Quick overview (tldr style)
vfab stats jobs [--json]        # Detailed job analytics
vfab stats performance          # Time usage and performance metrics
```
...
**CLI Interface**
- [x] `vfab stats summary` - Quick overview with JSON support
- [x] `vfab stats jobs` - Detailed job analytics
- [x] `vfab stats performance` - Time usage analytics
...
# Add a job
vfab job add drawing.svg --paper a4

# Plan and plot
vfab job plan drawing
vfab plot drawing
