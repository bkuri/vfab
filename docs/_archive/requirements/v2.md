# vfab v2 — Product Requirements Document (DEFERRED)

**One-liner:** Enhanced vfab with TUI interface, multi-device support, and native camera integration while preserving CLI-first architecture.

**Status:** 📋 **DEFERRED** - Planned for v2 development after v1 completion

**Targets**
- **Users:** vfab v1 users wanting enhanced interactive experience
...
**Goals** 📋 **PLANNED**
- 📋 **TUI (Terminal User Interface)** - Interactive terminal interface for vfab operations
- 📋 **Multi-device support** - Multiple plotter management and orchestration
- 📋 **Native camera support** - USB/v4l2 camera integration with custom ffmpeg filters
- 📋 **Enhanced performance** - Optimizations for large-scale operations
- 📋 **Advanced analytics** - Extended statistics and reporting capabilities
...
## 2) User Stories (v2) 📋 **PLANNED**

### 2.1 TUI User Stories

1. 📋 **Launch TUI** → User runs `vfab --tui` to enter interactive mode
2. 📋 **View Devices** → See connected plotter status and health metrics (multi-device)
...
7. 📋 **Device Health** → Monitor plotter health metrics (multi-device)
8. 📋 **Job Selection** → Interactive job browsing and selection for plotting

### 2.2 Multi-Device User Stories

9. 📋 **Device Discovery** → Automatically detect and configure multiple plotters
10. 📋 **Device Pooling** → Create device pools for load balancing
11. 📋 **Job Assignment** → Assign jobs to specific devices or device pools
12. 📋 **Concurrent Plotting** → Run multiple plotting jobs simultaneously on different devices
13. 📋 **Device Monitoring** → Monitor health and status of all connected devices
14. 📋 **Failover Handling** → Automatic job reassignment on device failure

### 2.3 Native Camera User Stories

15. 📋 **USB Camera Detection** → Automatically detect and configure USB/v4l2 cameras
16. 📋 **Custom Filters** → Apply perspective correction, color enhancement, noise reduction
17. 📋 **Hardware Acceleration** → Use GPU acceleration for video encoding
18. 📋 **Multi-Camera Support** → Support multiple cameras for different viewing angles
19. 📋 **Camera Health** → Monitor camera connectivity and feed quality

---

## 3) Architecture (v2) 📋 **PLANNED**

```
CLI/TUI ──> Orchestrator (FSM) ──> Device Manager (v2: multi-device)
│
├─> vpype Runner (subprocess)
├─> Estimator (geom + pyaxidraw)
├─> Capture (ffmpeg: IP v1 + v4l2 v2)
├─> Persistence (SQLite + workspace FS + JSONL journal)
├─> Statistics Engine (enhanced)
└─> Device Pool Manager (v2)
```
...
## 4) Data Model Extensions (v2) 📋 **PLANNED**
...
## 5) TUI Implementation (v2) 📋 **PLANNED**
...
```python
class PlottyTUI(App):
    """vfab Terminal User Interface v2"""
...
## 6) Multi-Device Implementation (v2) 📋 **PLANNED**
...
## 7) Native Camera Integration (v2) 📋 **PLANNED**
...
## 8) Enhanced Statistics (v2) 📋 **PLANNED**
...
## 9) Implementation Phases (v2) 📋 **PLANNED**

### Phase 1: TUI Foundation (Week 1-2)
1. **Add Textual dependency** to `pyproject.toml`
2. **Create integrated TUI module** at `src/vfab/tui.py`
...
## 10) Success Criteria (v2) 📋 **PLANNED**
...
## 11) Migration Strategy (v2) 📋 **PLANNED**
...
- Rollback procedures for v2 → v1 downgrade
...
## 14) v2 Development Status 📋 **PLANNED**
...
**v2 represents the next evolution of vfab**, adding interactive TUI capabilities, multi-device support, and native camera integration while maintaining the robust CLI-first architecture established in v1. Development will proceed after v1 achieves production stability and user feedback is incorporated.