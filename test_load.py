...
class LoadTestScenario:
    ...
    def generate_load_report(self):
        ...
        if 'database' in self.results:
            ...
            print(f"   Performance: {'✅ Excellent' if db['jobs_per_second'] > 1000 else '⚠️ Needs improvement' if db['jobs_per_second'] > 500 else '❌ Poor'}")
        ...
        if 'cli' in self.results:
            ...
            for cmd_name, results in self.results['cli'].items():
                if 'timeout' in results:
                    ...
                    status = '✅' if results['execution_time'] < 2.0 else '⚠️' if results['execution_time'] < 5.0 else '❌'
        ...
        # CLI scoring
        if 'cli' in self.results:
            cli_good = all(r.get('execution_time', 999) < 5.0 for r in self.results['cli'].values())
            if cli_good:
                performance_score += 1
        ...
        status = '✅ EXCELLENT' if percentage >= 75 else '⚠️ ACCEPTABLE' if percentage >= 50 else '❌ NEEDS IMPROVEMENT'
        
        print(f"   Performance Score: {performance_score}/{max_score} ({percentage:.0f}%)")
...
def main():
    ...
    parser.add_argument("--jobs", type=int, default=100, help="Number of jobs to test with")
